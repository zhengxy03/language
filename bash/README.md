...
* -x 精确匹配，完全匹配
## ln
* -s 创建符号链接 ln -s 目标目录(/mnt/d/mygit/..) 软连接(~/Scripts/..)
## split
...
* <<< 是一种重定向输入的操作符
## 括号
* \$\()\:命令替换（Command Substitution）：这是一种将命令的输出插入到另一个命令中的方法。\$() 内的命令会被执行，其输出会被替换到原位置。
* ${}变量替换
* [[]]:条件测试：[[ ]] 是 Bash 的条件测试语句，用于测试条件表达式。
* (()):算术扩展：(( )) 是 Bash 的算术扩展，用于执行数学运算。它允许使用更复杂的数学表达式。
## POSIX字符类
